...
import (
	...
	"github.com/heindl/migrate/v4/source"
...
func (g *Gitlab) OpenWithValues(baseUrl, projectID, token, migrationsPath, ref string) (source.Driver, error) {
...
func (g *Gitlab) Open(url string) (source.Driver, error) {
	...
	client, err := gitlab.NewClient(password, gitlab.WithBaseURL(uri.String()))
	if err != nil {
		return nil, err
	}
